...
void match_finder::operator()() {
    ...
    if (file_size() > MAX_FILE_SZ) {
        ...
        print_match(filepath,-1,msg,std::cerr);
        return;
    }
    ...
    auto m = get_matches(
            std::string(it,eos), 
            std::regex(input_ptrn)
            );

    for (const auto& e : m) {
        std::string res (e.second[0]);
        for (const auto& e2 : e.second) {
            res = merge_results(res, e2);
        }
        print_match(filepath,e.first,res,std::cout);
    }
}
