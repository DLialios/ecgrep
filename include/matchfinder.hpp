...
class match_finder {
    ...
    *  @return Map of line numbers to pair of bool and vector. 
    *          Vector contains matches for this line. Bool denotes 
    *          whether matches for this line are too long to print.
    *          Match results are colored. */
    std::map<int, std::pair<bool,std::vector<std::string>>> 
    get_matches(const std::string &str);
    ...
    *  @return String with match highlighted. */
    std::string 
    color_output_string(const std::string &s,
    ...
    *  Variant of longest common substring problem. */
    int 
    find_match_index(const std::string &s, 
                     const std::string &suffix);

    /* @return String preserving the highlights from
    *          two separate matches. */
    std::string 
    merge_results(const std::string &s1,
                  const std::string &s2);
    ...
    *          an ANSI terminal code in @a s. */
    ...
    get_term_ctrl_loc(const std::string &s,
                      int offset);
...
public:
    match_finder(const std::string &filepath, 
