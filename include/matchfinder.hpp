...
template <typename C, typename V> auto find_all(C &&c, const V &v) {
...
class match_finder {
   ...
   *         colored. */
    std::map<int, std::vector<std::string>> get_matches(const std::string &str);
   ...
   * @return String with match highlighted. */
    std::string color_output_string(const std::string &s,
                                    const std::string &match,
                                    const std::string &suffix);

    /* @return Index in @a s where @a suffix begins.
   * Variant of longest common substring problem. */
    int find_match_index(const std::string &s, const std::string &suffix);

    /* @return String preserving the highlights from
   *         two separate matches. */
    std::string merge_results(const std::string &s1, const std::string &s2);
   ...
   *         an ANSI terminal code in @a s. */
    ...
    get_term_ctrl_loc(const std::string &s, int offset);
   ...
   *         file descriptor to write it to. */
    std::pair<bool, std::string> print_match(const int lineno, std::string &line,
                                             const bool is_err);
...
public:
    match_finder(const std::string &filepath, const std::string &pwdpath,
                 const std::regex &patrn);
   ...
   * This operation loads the file into memory. */
    std::vector<std::pair<bool, std::string>> operator()();
};
